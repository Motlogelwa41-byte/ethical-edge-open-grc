...
class NetworkTelemetry(Base):
    ...
    node_status = Column(String, nullable=False)  # OPTIMAL_HEALTH, SECURITY_BREACH_VULNERABILITY, DEGRADED_PERFORMANCE
    synchronized_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    telemetry_metadata = Column(JSON, nullable=True)  # Solar battery tracking, tenant ID token references
