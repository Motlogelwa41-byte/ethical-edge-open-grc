...
class GRCEvidenceEngine:
    ...
    def verify_aws_mfa_control(self) -> ControlVerificationResult:
        """
        Maps to ISO 27001:2022 A.8.5 (Secure Authentication) & BDPA Access Control principles.
        Verifies if IAM Users have MFA active.
        """
        try:
            # In production, credentials would be dynamically retrieved securely per tenant/client
            ...
            evidence = {
                ...
                "assessment_notes": "All users must have an active MFA virtual or hardware token."
            }
            ...
            # Fallback/Error capture for dashboard transparency if integration fails
            return ControlVerificationResult(
                control_reference="A.8.5",
                control_name="Multi-Factor Authentication Enforcement",
                framework="ISO/IEC 27001:2022",
    ...
    def verify_github_repository_privacy(self, org_name: str, github_token: str) -> ControlVerificationResult:
        """
        Maps to NIST CSF 2.0 PR.DS-01 (Data Loss Prevention) & POPIA / BDPA Information Protection.
        ...
        try:
            ...
            # If your company policy dictates ZERO public repositories in this workspace
            is_compliant = len(public_repos) == 0
            status = "PASSED" if is_compliant else "FAILED"
            
            evidence = {
                "public_repo_count": len(public_repos),
                "exposed_repositories": [repo['name'] for repo in public_repos][:5], # limit size
